...
| NOAA/NCEI Local Climatological Data | https://www.ncei.noaa.gov/products/land-based-station/local-climatological-data | Explicit Central Park hourly precipitation fallback | Approved fallback; downloaded and loader-tested | `DATE`, `REPORT_TYPE`, `HourlyPrecipitation`, station id/name, hourly weather fields | LCDv2 contains hourly observations and hourly precipitation; Central Park yearly CSV files exist in NCEI object storage | Fallback, not original preferred NRCC source; use `FM-15` hourly observations, bucket `:51` timestamps to the hour, preserve missing rainfall | Yes, approved explicit fallback for V1 |
...
| --- | --- | --- | --- | --- | --- |
| Version 1 feature store | NRCC/Central Park remains preferred conceptually; NOAA LCD Central Park `USW00094728` is approved as the practical fallback | NOAA LCD if useful and non-blocking | NOAA CO-OPS 8518750 The Battery | DEP reports for methodology | Build first local hourly external-forcing dataframe |
